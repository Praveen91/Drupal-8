...
class MydataForm extends FormBase {
	...
	$form['candidate_name'] = array(
      ...
      '#description' => $this->t('Your name without space.'),
      );
    //print_r($form);die();
	...
	public function validateForm(array &$form, FormStateInterface $form_state) {
		...
		if (preg_match('/[^A-Za-z]/', $name)) {
			$form_state->setErrorByName('candidate_name', $this->t('Your Name must in character without space'));
		}

		// Confirm age is nemeric.
		$age = $form_state->getValue('candidate_age');
		if (!intval($age)) {
			$form_state->setErrorByName('candidate_age', $this->t('Your age must be Nemeric.'));
		}
    ...
    public function submitForm(array &$form, FormStateInterface $form_state) {
    	...
    	if (isset($_GET['num'])) {
	    	...
	    	$form_state->setRedirect('mydata.mydate_controller_table');
    	}
    	...
    	$response = new RedirectResponse("/mydata/table");
        $response->send();
